...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="llama_7B",
        choices=HF_NAMES.keys(),
        help="model name",
    )
    parser.add_argument(
        "--use_honest",
        action="store_true",
        help="use local editted version of the model",
        default=False,
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
        default="tqa_mc2",
        help="feature bank for training probes",
    )
    parser.add_argument(
        "--activations_dataset",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=48,
        help="K, number of top heads to intervene on",
    )
    parser.add_argument(
        "--alpha", type=float, default=15, help="alpha, intervention strength"
    )
    ...
    parser.add_argument(
        "--use_center_of_mass",
        action="store_true",
        help="use center of mass direction",
        default=False,
    )
    parser.add_argument(
        "--use_random_dir",
        action="store_true",
        help="use random direction",
        default=False,
    )
    ...
    # create model
    model_name = HF_NAMES[
        "honest_" + args.model_name if args.use_honest else args.model_name
    ]
    ...
    activations_dataset = (
        args.dataset_name
        if args.activations_dataset is None
    ...
    for i in range(args.num_fold):
        train_idxs = np.concatenate(
            [fold_idxs[j] for j in range(args.num_fold) if j != i]
        )
        ...
        train_set_idxs = np.random.choice(
            train_idxs, size=int(len(train_idxs) * (1 - args.val_ratio)), replace=False
        )
        ...
        df.iloc[val_set_idxs].to_csv(
            f"splits/fold_{i}_val_seed_{args.seed}.csv", index=False
        )
        df.iloc[test_idxs].to_csv(
            f"splits/fold_{i}_test_seed_{args.seed}.csv", index=False
        )
        ...
        def lt_modulated_vector_add(head_output, layer_name, start_edit_location="lt"):
            ...
            for head, direction, proj_val_std in interventions[layer_name]:
                ...
                if start_edit_location == "lt":
                    head_output[:, -1, head, :] += (
                        args.alpha * proj_val_std * direction_to_add
                    )
                else:
                    head_output[:, start_edit_location:, head, :] += (
                        args.alpha * proj_val_std * direction_to_add
                    )
    ...
    print(
        f"True*Info Score: {final[1]*final[0]}, True Score: {final[1]}, Info Score: {final[0]}, MC1 Score: {final[2]}, MC2 Score: {final[3]}, CE Loss: {final[4]}, KL wrt Original: {final[5]}"
    )
